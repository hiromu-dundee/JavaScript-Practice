...
const Weather = function (weatherCode) {
    ...
    } else if ((61 <= weatherCode && weatherCode <= 65) || (80 <= weatherCode && weatherCode <= 82)) {
...
window.addEventListener('load', () => {
    fetch('https://api.open-meteo.com/v1/forecast?latitude=35.6895&longitude=139.6917&daily=weathercode,precipitation_sum,temperature_2m_max,temperature_2m_min&current=temperature_2m,relative_humidity_2m,wind_speed_10m&timezone=Asia%2FTokyo')
        ...
        .then((forecasts) => {
            ...
            document.querySelector('.humid').textContent = `湿度：${forecasts.current.relative_humidity_2m}%`;
            document.querySelector('.wind').textContent = `風速：${forecasts.current.wind_speed_10m}m`;
            ...
            for (let i = 1; i < forecasts.daily.time.length; i++) {
                ...
                const [year, month, day] = forecasts.daily.time[i].split('-').map(Number);
                const Day = new Date(year, month - 1, day);
                const code = forecasts.daily.weathercode[i];
                const [temp_max, temp_min] = [forecasts.daily.temperature_2m_max[i], forecasts.daily.temperature_2m_min[i]];
